...
class EncoderRNN(nn.Module):
    ...
    def forward(self, input, lengths, hidden):
        ...
        # 给定输入
        assert isinstance(self.embedding,nn.modules.sparse.Embedding), "line28:self.embedding must be an instance of nn.Embedding"

        # 打印嵌入维度和GRU input_size以确认它们是否匹配
        print(f"Embedding dim: {self.embedding.embedding_dim}, GRU input_size: {self.gru.input_size}")

        input_embedded = self.embedding(input)  # => batchSize × seq_len × feaSize
        lengths = lengths.to('cpu', dtype=torch.int64)
        # 加入paddle 方便计算
        packed = nn.utils.rnn.pack_padded_sequence(input_embedded, lengths, batch_first=True)
...
class LuongAttentionDecoderRNN(nn.Module):
    ...
    def forward(self, inputStep, hidden, encoderOutput):
        ...
        # 对输入做dropout
        inputStep_embedded = self.embedding(inputStep)  # => batchSize × 1 × feaSize
        inputStep_dropped = self.dropout(inputStep_embedded)
        output, hidden = self.gru(inputStep_dropped,
...
class Seq2Seq:
    ...
    def __init__(self, dataClass,word2vec_modelPath,featureSize, hiddenSize, encoderNumLayers=1, decoderNumLayers=1,
                 dropout=0.1, outputSize=None, learning_rate=0.001,embedding=None,
        ...
        self.word2vec_model = Word2Vec.load(word2vec_modelPath)
        # 获取词汇表大小和嵌入维度
        vocabulary_size = len(self.word2vec_model.wv.key_to_index)
        embedding_dim = self.word2vec_model.vector_size
        # 创建一个空的嵌入层
        self.embedding = nn.Embedding(vocabulary_size, embedding_dim)
        # 将 Word2Vec 模型的权重赋值给嵌入层的权重
        self.embedding.weight.data.copy_(torch.from_numpy(self.word2vec_model.wv.vectors))
        self.embedding = self.embedding.to(device)
        assert isinstance(self.embedding,
                          nn.modules.sparse.Embedding), "line142:self.embedding must be an instance of nn.Embedding"
        self.encoderRNN = EncoderRNN(featureSize, hiddenSize, embedding=self.embedding, numLayers=encoderNumLayers,
                                     dropout=dropout, bidirectional=True).to(device)

        self.decoderRNN = LuongAttentionDecoderRNN(featureSize, hiddenSize, outputSize, embedding=self.embedding,
    ...
    def train(self, batchSize, epoch=100, stopRound=10, betas=(0.9, 0.99), eps=1e-08, weight_decay=0,
        ...
        eosToken = self.dataClass.word2id['<EOS>']
        sosToken = self.dataClass.word2id['<SOS>']
        # 给定batchSize和是否数据增广
            ...
            for i in range(itersPerEpoch):
                ...
                if (e * itersPerEpoch + i + 1) % stopRound == 0:
                    bleu = _bleu_score(self.encoderRNN, self.decoderRNN, X, XLens, Y, YLens, self.dataClass.maxSentLen,
                                       device=self.device, eosToken=eosToken, sosToken=sosToken)
                    ...
                    if self.dataClass.testSize > 0:
                        ...
                        bleu = _bleu_score(self.encoderRNN, self.decoderRNN, X, XLens, Y, YLens,
                                           self.dataClass.maxSentLen, device=self.device, eosToken=eosToken,
                                           sosToken=sosToken)
...
class ChatBot:
    ...
    def predictByBeamSearch(self, inputSeq, beamWidth=100, maxAnswerLength=50, alpha=0.7, isRandomChoose=False,
        ...
        sosToken = self.word2id['<SOS>']
        outputSize = len(self.id2word)
        inputSeq = filter_sent(inputSeq)
        inputSeq = ['<SOS>'] + [w for w in tokenize(inputSeq) if w in self.word2id.keys()] + ['<EOS>']  # 分词
        ...
        d = 2  # Adjust for bidirectional GRU
    ...
    def evaluate(self, dataClass, batchSize=128, streamType='train'):
        ...
        while True:
            ...
            bleuScore += _bleu_score(self.encoderRNN, self.decoderRNN, X, XLens, Y, YLens, dataClass.maxSentLen,
                                     self.device, eosToken=eosToken, sosToken=sosToken)
            batchLoss, batchNTotal = _calculate_loss(self.encoderRNN, self.decoderRNN, X, XLens, Y, YLens,
                                                     teacherForcingRatio=0, device=self.device, eosToken=eosToken,
                                                     sosToken=sosToken)
...
def _bleu_score(encoderRNN, decoderRNN, X, XLens, Y, YLens, maxSentLen, device, eosToken, sosToken):
    Y_pre = _calculate_Y_pre(encoderRNN, decoderRNN, X, XLens, Y, maxSentLen, teacherForcingRatio=0, device=device,
                             eosToken=eosToken, sosToken=sosToken)
...
# 计算loss
def _calculate_loss(encoderRNN, decoderRNN, X, XLens, Y, YLens, teacherForcingRatio, device, eosToken, sosToken):
    ...
    print("X size:", X.size())  # 应为 [batchSize, XSeqLen]
    print("XLens:", XLens)  # 每个序列的实际长度
    print("hidden size:", hidden.size())  # 应为 [d * numLayers, batchSize, hiddenSize]
...
# 计算Y的预测值
def _calculate_Y_pre(encoderRNN, decoderRNN, X, XLens, Y, YMaxLen, teacherForcingRatio, device, eosToken, sosToken):
    print("460eos=", eosToken)
    print("460sos=", sosToken)
